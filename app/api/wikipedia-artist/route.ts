...
export async function GET(request: NextRequest) {
  ...
  try {
    // Try to find the Wikipedia page for the artist
    // First, try searching for the artist name directly
    const searchQueries = [
      artist,
      `${artist} (musician)`,
      `${artist} (band)`,
    ...
    if (!extract) {
      const searchApiUrl = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(artist)}`;
      try {
